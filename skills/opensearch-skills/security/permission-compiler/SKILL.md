...
cluster, explicitly add `--allow-private-target`; this does not permit cloud
metadata or redirect targets. Plaintext HTTP remains limited to literal
loopback addresses for local development. Redirects are recorded but never
followed, so credentials are not replayed to a `Location` target.
